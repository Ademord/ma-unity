...
public class Visualization : MonoBehaviour
{
    ...
    private void DrawBounds()
    {
        ...
        foreach (OctreeNode node in nodes)
        {
            if (node.GetPoints(pointFilter, points))
            {
                ...
                PopGizmos.Cube(node.Center, Quaternion.identity, size, col);
            }
        }
    }
    ...
    private void DrawPoints()
    {
        ...
        foreach (OctreeNode node in nodes)
        {
            if (node.GetPoints(pointFilter, points))
            {
                ...
                {
                    PopGizmos.Cube(point.Position, Quaternion.identity, size, colors[point.Type]);
                }
            }
        }
    }
    ...
    private void DrawScanDensity()
    {
        ...
        foreach (OctreeNode node in nodes)
        {
            ...
            PopGizmos.Line(node.Center, node.Center + line, cg.GetColor(t));
        }
    }
...
public class LeafNodeDrawer
{
    ...
    public void Draw(Vector3 pos, int val, Color col)
    {
        for (int i = 0; i < 12; i++)
        {
            ...
            PopGizmos.Line(pos + this.edges[i][0], pos + this.edges[i][1], c);
        }
    }
}
